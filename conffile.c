...
int parse_config_file(struct io_cfg * config,char * filename){
	...
	if (!yaml_parser_load(&parser,&document)){
		dprint(ERR,"Could not load config file to YAML parser (probably syntax error)\n");
		return -1;
	}
	fclose(cfg_file);
	...
	if (root->type != YAML_SEQUENCE_NODE){
		dprint(ERR,"Wrong type of YAML root node (must be sequence)\n");
		return -1;
	}
	...
	for (int i=0; i<items; i++){
		...
		if (ep_params->type != YAML_MAPPING_NODE){
			dprint(ERR,"Wrong type of YAML sequence node (must be mapping)\n");
			return -1;
		}
